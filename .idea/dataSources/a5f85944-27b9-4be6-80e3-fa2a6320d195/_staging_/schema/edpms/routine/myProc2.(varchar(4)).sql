...
CREATE PROCEDURE myProc2(IN val VARCHAR(15), IN val2 VARCHAR(15))
  BEGIN
     DROP TEMPORARY TABLE IF EXISTS ttab;
     create TEMPORARY TABLE IF NOT EXISTS ttab (SELECT projectdetails.PROJECTID, projectdetails.LGA, projectdetails.CONTRACTSUM, (projectdetails.DATEOFAWARD) as yr,
     ...
     (SELECT LGA, SUM(CONTRACTSUM) as CSUM1, SUM(cAmount) as CAMOUNT, SUM(vAmount) as VAMOUNT from ttab WHERE yr BETWEEN val AND val2 GROUP BY LGA);
